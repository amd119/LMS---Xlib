...
class BukuController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Validasi data selain file Cover
            ...
            $currentCover = $buku->Cover;
            if ($request->hasFile('Cover')) {
                $coverFile = $request->file('Cover');

                // Validasi file Cover
                $validatedData['Cover'] = $coverFile->getClientOriginalName();

                // Hapus Cover lama jika ada
                if ($currentCover && Storage::exists('public/uploads/' . $currentCover)) {
                    Storage::delete('public/uploads/' . $currentCover);
                }

                // Simpan file Cover baru
                $img = Image::make($coverFile)->encode('jpg');
                Storage::put('public/uploads/' . $validatedData['Cover'], $img);
            } else {
                // Jika pengguna tidak mengunggah file Cover baru, pertahankan Cover yang ada
                $validatedData['Cover'] = $currentCover;
            }
